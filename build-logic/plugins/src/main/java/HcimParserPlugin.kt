...
class HcimParserPlugin : Plugin<Project> {
  override fun apply(target: Project) {
    target.tasks.register("updateHcimParser") {
      try {
        // Files are downloaded to this directory
        val workingDir = File(target.rootDir, "hcimParserTmp")
        workingDir.mkdir()
  ...
  private fun Project.moveFiles(workingDir: File) {
    ...
    println("Downloaded HCIM Parser. Version: ${targetVersionFile.readText()}")
    val destinationVersionFileMain = File(project.rootDir, "data/hcimParser/src/main/assets/mgo-fhir-data.iife.version.json")
    val destinationVersionFileTestFixtures = File(project.rootDir, "data/hcimParser/src/testFixtures/resources/mgo-fhir-data.iife.version.json")
    ...
    val destinationJsFileMain = File(project.rootDir, "data/hcimParser/src/main/assets/mgo-hcim-api.iife.js")
    val destinationJsFileTestFixtures = File(project.rootDir, "data/hcimParser/src/testFixtures/resources/mgo-hcim-api.iife.js")
    targetJsFile.copyTo(destinationJsFileMain, overwrite = true)
    targetJsFile.copyTo(destinationJsFileTestFixtures, overwrite = true)
  }
  ...
  private fun Project.generateKotlinClasses(jsonSchema: JSONObject) {
    ...
    val kotlinClassesDir = File(rootDir, "data/hcimParser/src/main/java/nl/rijksoverheid/mgo/data/hcimParser/uiSchema/models")
    kotlinClassesDir.deleteRecursively()

    // Generate kotlin classes based on the json schema
    CodeGenerator().apply {
      baseDirectoryName = File(project.rootDir, "data/hcimParser/src/main/java").path
  ...
  private fun Project.makeInterfacesSealed() {
    val directory = File(rootDir, "data/hcimParser/src/main/java/nl/rijksoverheid/mgo/data/hcimParser/uiSchema/models")
  ...
  private fun Project.makeProfilesClassStatic() {
    val file = File(rootDir, "data/hcimParser/src/main/java/nl/rijksoverheid/mgo/data/hcimParser/uiSchema/models/Profiles.kt")
