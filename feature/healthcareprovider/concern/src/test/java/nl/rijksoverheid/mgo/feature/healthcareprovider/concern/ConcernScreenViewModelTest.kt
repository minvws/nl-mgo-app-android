...
internal class ConcernScreenViewModelTest {
    ...
    fun `Given concerns, When creating viewmodel, View state is updated`() =
        runTest {
            ...
            viewModel.viewState.test {
                Assert.assertEquals(concerns, (awaitItem() as ConcernScreenViewState.Success).concerns)
            }
        }
    ...
    fun `Given error, When creating viewmodel, View state is updated`() =
        runTest {
            ...
            // Then
            viewModel.viewState.test {
                Assert.assertEquals(error, (awaitItem() as ConcernScreenViewState.Error).error)
            }
        }
}
