...
class DefaultHealthCareDataStatesStoreTest {
  ...
  fun testObserveCategory() =
    runTest {
      ...
      store.observe(category = HealthCareCategory.MEDICATIONS, filterOrganization = null).test {
  ...
  fun testObserveCategoryAndOrganization() =
    runTest {
      ...
      store.observe(category = HealthCareCategory.MEDICATIONS, filterOrganization = TEST_MGO_ORGANIZATION.copy(id = "1")).test {
